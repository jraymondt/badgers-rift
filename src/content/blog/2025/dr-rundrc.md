---
title: run dmc ham master 
description: a guide to the universe
pubDate: 2024-08-17
heroImage: '/img/2025/dr_zoom.png'
tags:
  - zoomer
  - run
  - dr
  - cam
  - jam
  - jim
---

## Perth Daily Reflections audio-only meeting of Alcoholics Anonymous

![shadow bun](/img/2025/shadow_bun.png)

### Pre-meeting set-up

Arrive to set-up 15 mins beforehand to allow for any login/tech issues.
(If a designated host has not opened the meeting by 6.50am, any member who has previously hosted may login to start the meeting.)

### Login and start meeting

1. Login to Zoom using
User ID ~~redacted~~, passcode ~~redacted~~
*If Zoom asks for a code at login, which sometimes happens if you’re logging in from somewhere other than your usual IP address, you will need to access a one-time access code from the email account, password is also ~~redacted~~
2. Join Daily Reflection Meeting using: Meeting ID 82166401300 and passcode ~~redacted~~

### Apply Zoom meeting settings (via ‘host tools’)

1. Host settings

- In ‘host settings’ uncheck all permissions except ‘unmute’ and ‘chat’ so people can talk before the 7am start.
- The ‘rename’ function can be hijacked by bombers but it also lets people change their own names, restrict at host discretion.

### Chat

- Set chat to ‘everyone’ by clicking the chat icon then the 3 dots (…) to stop direct messages.
- If there are a few unknown attendees that look ‘interesting’, group suggestion is to set chat to ‘host and co-host’ only for the first half of the meeting then open it up to ‘everyone’ later when things settle. (For context bombing can include unmuting and having porn on in the background, trying to start their video, again with porn on, and using the rename function to be offensive, yes, the mind boggles.)
- Copy and paste Daily Reflection into chat from [https://www.aa.org/daily-reflections](https://www.aa.org/daily-reflections) continue to post as needed noting that attendees can only see messages in chat from the time they enter the meeting.

### Allocate co-hosts

It’s good to have one or more co-hosts to

- Admit people into the meeting
- Copy and paste the daily reflection into chat during the meeting
- Monitor the chat
- Welcome any newcomers and post relevant information
- Post the 7th tradition details

### Allocate readers

[Preamble - back of any Serenity      ->](https://www.aa.org/sites/default/files/literature/smf-92_en.pdf)
[How it Works (pp. 58-60 Big Book)    ->](https://www.aa.org/sites/default/files/2021-11en_bigbook_chapt5.pdf)
[Daily reflection, and first share    ->](https://www.aa.org/daily-reflections)

- Preamble:             _______________
- How it Works:         _______________
- Daily Reflection:     _______________

- Welcome to the Perth Daily Reflection meeting of Alcoholics Anonymous - my name is JIM and I AM an alcoholic
- This is a 30 min audio only meeting, 7AM AWST, 7 days a week
- This group encourages attendance to face-to-face meetings and sponsorship so if you’re new to AA, please stay after the meeting so members can chat with you, a link to the Australian AA meeting website will be posted in chat, along with details of this groups WhatsApp so that people can stay connected (copy and paste the below and post into chat):

- Preamble  
- How it Works  

## Housekeeping  

- We are set to mute during the meeting and will be sent a request to unmute when asked to share, No cameras in this meeting.
- Chat functionality is restricted to hosts only during first half of the meeting and opened about halfway through as the meeting settles.
- 7th tradition: This meeting doesn’t have capacity to take contributions, feel free to contribute to your next in person meeting or to the AA general service office via details in chat.
- If you’d like to become a host or co-host, please let us know after the meeting or come in early and ask the meeting hosts.
- ‘proof of attendance’ for anyone who needs it, you can message us directly.
- Finally, this is a topic meeting based on the AA Daily Reflection book, shares of about 2-3 minutes help us to get to as many people as possible.
- Daily reflection and first share

---------

```text

7th Tradition Details:
 
PayID:          donations@aa.org.au 
 
PayPal:         paypal.me/AAGSOAustralia
 
Electronic Transfer:
BSB:            012006 
Account number: 009750529
Account name:   General Service Board of Alcoholics Anonymous Australia 
 
Please post cheques or money orders to:
General Service Office of AA
48 Firth St
Arncliffe NSW 2205
```

---------

## Daily Reflect your fasciae off please

- Choose people to share and send request to unmute
a. Mention using the ‘raise hand’ function for people that ‘have a burning desire’ to share
b. Choose members to maximise engagement and inclusion (host discretion)
- 7th Tradition (at approx. 7:15) - This meeting doesn’t have capacity to take contributions, feel free to contribute to your next in person meeting or to the AA general service office via details in chat

Chat post to new members:

```text
❤️ To anyone who may be struggling... Please raise your hand and share with us! We can better support you if we can hear from you.  If you feel unable to do this, please private message the group members and we can still help you ❤️

Staying connected:
This group has a WhatsApp chat, feel free to join, this is a good way to get some numbers and get support to go to an in-person meeting: https://chat.whatsapp.com/Ja5rlLJrUvCHvaGgzoEPZj

Getting to an in-person meeting:
AA meetings list: https://meetings.aa.org.au/regions/
```

---------

### Requests for ‘proof of attendance’ (sometimes for court/rehab reasons)

A meeting host or co-host may provide the following details in chat directly for the participant to screenshot:

```text

Please screenshot for proof of attendance:

_______________________________
Attended the Perth Daily Reflection AA meeting on 22/08/2025
Host: Jim (James Thompson) 

```

## AA Stuff

- [The Big Book of Alcoholics Anonymous:](https://www.aa.org/the-big-book)
- [Everything AA App:](https://www.tanukitech.dev/everything-aa)
- [12 steps & 12 traditions pdf:](https://www.aa.org/twelve-steps-twelve-traditions)
- [The 24hr Marathon Meeting Zoom ID: 2923712604, no password required. Never shuts, open 24/7, new meeting on the hour, every hour, since 20-4-20. Members from all over Globe. Direct link to the meeting:](https://us02web.zoom.us/j/2923712604)

“In a world where we can be anything, be kind"
Bertie Watts 🙏💖

### term

```text

11.2 Restrictions on Use by Children. Zoom is not intended for use by individuals under the age of sixteen (16) years old, unless it is through a School Subscriber (as defined in the Zoom Services Description) using Zoom for Education (K-12).

19. APPLE iOS TERMS OF USE
By accessing or downloading a Zoom application from the Apple App Store, you are agreeing to Apple’s Licensed Application End User License Agreement (“Apple Terms”). This Agreement governs if there is a conflict with the Apple Terms.
ZOOM AND ITS AFFILIATES AND EACH OF THEIR LICENSORS, AND SUPPLIERS WILL NOT BE LIABLE FOR ANY:
- SPECIAL, INCIDENTAL, INDIRECT, CONSEQUENTIAL, EXEMPLARY, OR PUNITIVE DAMAGES;
- LOSS OF BUSINESS PROFITS, BUSINESS INTERRUPTION, LOSS OF BUSINESS INFORMATION, LOSS OF BUSINESS OPPORTUNITY;
- UNAUTHORIZED ACCESS TO, LOSS OF, DELETION OF, OR ALTERATION OF SYSTEM DATA, CUSTOMER CONTENT, OR CUSTOMER DATA;
- COSTS RELATED TO THE PROCUREMENT OF SUBSTITUTE GOODS OR SERVICES;
- TERMINATION, SUSPENSION, DISCONTINUANCE, OR DISCONNECTION OF THE SERVICES;
- A FAILURE OF YOUR INTERNET SERVICES, DOWNTIME, OR MAINTENANCE;
- OUR FAILURE TO PROVIDE TECHNICAL OR OTHER SUPPORT SERVICES; OR
- DAMAGES, IN THE AGGREGATE FOR ALL CLAIMS ARISING OUT OF OR RELATED TO THIS AGREEMENT, EXCEEDING THE AMOUNT ACTUALLY PAID BY YOU FOR THE SERVICES (IF ANY) IN THE TWELVE (12) MONTHS PRECEDING THE EVENT OR CIRCUMSTANCES GIVING RISE TO SUCH CLAIMS.

Also, this website should be consumed with a tea, or not at all.

```
